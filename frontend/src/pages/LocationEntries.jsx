...
const LocationEntries = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.locationCode.trim() || !formData.locationName.trim()) { 
    ...
    try {
      setSaving(true);
      if (isEditing) { 
        await axios.put(`${BASE_URL}/api/locations/${editId}`, { locationCode: formData.locationCode, locationName: formData.locationName }); 
        toast.success('Location updated successfully'); 
      } else { 
        await axios.post(`${BASE_URL}/api/locations`, { locationCode: formData.locationCode, locationName: formData.locationName }); 
  ...
  const editItem = (item) => { 
    setIsEditing(true); 
    setEditId(item._id); 
    setFormData({ recordId: item.recordId, locationCode: item.locationCode, locationName: item.locationName }); 
  };

  const resetForm = () => { 
    setIsEditing(false); 
    setEditId(null); 
    setFormData({ recordId: 'Auto-generated', locationCode: '', locationName: '' }); 
  };
  ...
  const handleDelete = async () => {
    try { 
      await axios.delete(`${BASE_URL}/api/locations/${itemToDelete._id}`); 
    ...
    <div className="space-y-6 text-[#E6EDF3] animate-fade-in pb-10">
      ...
      <section className="flex justify-between items-center bg-[#161B22]/40 p-4 border border-white/5 rounded-xl">
        <div>
          ...
          <p className="text-xs font-mono text-[#8B949E] mt-0.5">Location Configuration Interface • [v.2.4.0]</p>
        </div>
        <button 
          onClick={downloadCSV}
          ...
          <div className="overflow-x-auto">
            {loading ? (
                ...
                <tbody className="divide-y divide-white/5">
                  {paginatedLocations.map((item) => (
                    <tr key={item._id} className="hover:bg-[#2563EB]/5 transition-colors group">
                      ...
                      <td className="px-6 py-3 text-sm font-bold text-[#E6EDF3] whitespace-nowrap">{item.locationCode}</td>
                      <td className="px-6 py-3 text-sm text-[#E6EDF3] whitespace-nowrap">{item.locationName}</td>
              ...
              <div className="p-12 text-center text-[#8B949E] font-mono text-xs">
                {searchTerm ? 'No matching facility locations found.' : 'Telemetry ledger empty. Configure new facility warehouse nodes.'}
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-[#161B22] border border-white/5 rounded-xl p-4 flex items-center gap-4 skeuo-shadow">
          ...
          <div>
            <p className="text-[9px] font-mono text-[#8B949E] uppercase tracking-wider">Total Locations</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#161B22] border border-white/5 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-[#E6EDF3] mb-2 font-mono uppercase tracking-wider">Delete Location Entry</h3>
              <p className="text-sm text-[#8B949E] mb-6">
                Are you sure you want to delete <span className="font-bold text-[#E6EDF3] font-mono">{itemToDelete?.recordId} ({itemToDelete?.locationCode})</span>? This action cannot be undone and will fail if any items are currently located inside this node.
              </p>
