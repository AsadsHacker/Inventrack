...
const SupplierEntry = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      if (isEditing) { 
        await axios.put(`${BASE_URL}/api/suppliers/${editId}`, { supplierName: formData.supplierName }); 
        toast.success('Supplier updated successfully'); 
      } else { 
        await axios.post(`${BASE_URL}/api/suppliers`, { supplierName: formData.supplierName }); 
  ...
  const editItem = (item) => { 
    setIsEditing(true); 
    setEditId(item._id); 
    setFormData({ recordId: item.recordId, supplierName: item.supplierName }); 
  };

  const resetForm = () => { 
    setIsEditing(false); 
    setEditId(null); 
    setFormData({ recordId: 'Auto-generated', supplierName: '' }); 
  };
  ...
  const handleDelete = async () => {
    try { 
      await axios.delete(`${BASE_URL}/api/suppliers/${itemToDelete._id}`); 
  ...
  const downloadCSV = () => {
    ...
    const headers = ["ID", "SUPPLIER NAME", "CREATED AT"];
    const csvRows = [
      headers.join(','),
      ...data.map(s => [
        `"${s.recordId}"`,
    ...
    link.href = url;
    link.setAttribute('download', 'inventory_suppliers.csv');
  ...
  const getLastIngestion = () => {
    ...
    const dates = data.map(s => s.createdAt ? new Date(s.createdAt).getTime() : 0).filter(d => d > 0);
    ...
    <div className="space-y-6 text-[#E6EDF3] animate-fade-in pb-10">
      ...
      <section className="flex justify-between items-center bg-[#161B22]/40 p-4 border border-white/5 rounded-xl">
        <div>
          ...
          <p className="text-xs font-mono text-[#8B949E] mt-0.5">Supplier Configuration Interface • [v.2.4.0]</p>
        </div>
        <button 
          onClick={downloadCSV}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                placeholder="E.g. Acme Corp, Global Steel"
                required
            ...
            <div className="bg-[#1C2128] border border-white/5 p-3 rounded-lg flex items-start gap-2.5 text-[10px] text-[#8B949E]">
              ...
              <span>Ensure supplier contact records and corporate names align with purchasing invoices to facilitate clean accounting audits.</span>
            </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
                ...
                <tbody className="divide-y divide-white/5">
                  {paginatedSuppliers.map((item) => (
                    <tr key={item._id} className="hover:bg-[#2563EB]/5 transition-colors group">
                      ...
                      <td className="px-6 py-3 text-sm font-bold text-[#E6EDF3] whitespace-nowrap">{item.supplierName}</td>
              ...
              <div className="p-12 text-center text-[#8B949E] font-mono text-xs">
                {searchTerm ? 'No matching supplier records found.' : 'Telemetry ledger empty. Configure new corporate suppliers.'}
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-[#161B22] border border-white/5 rounded-xl p-4 flex items-center gap-4 skeuo-shadow">
          ...
          <div>
            <p className="text-[9px] font-mono text-[#8B949E] uppercase tracking-wider">Total Suppliers</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#161B22] border border-white/5 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-[#E6EDF3] mb-2 font-mono uppercase tracking-wider">Delete Supplier Entry</h3>
              <p className="text-sm text-[#8B949E] mb-6">
                Are you sure you want to delete <span className="font-bold text-[#E6EDF3] font-mono">{itemToDelete?.recordId} ({itemToDelete?.supplierName})</span>? This action cannot be undone and will fail if the supplier is linked to existing transactions.
              </p>
