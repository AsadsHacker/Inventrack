...
import { 
  Edit2, Trash2, Search, Loader2, Info, Lock, Download, Database, ShieldAlert
} from 'lucide-react';
...
const UnitEntry = () => {
  ...
  const downloadCSV = () => {
    ...
    const headers = ["ID", "UNIT NAME", "CREATED AT"];
    const csvRows = [
      headers.join(','),
      ...units.map(u => [
        `"${u.recordId}"`,
  ...
  const getLastIngestion = () => {
    if (!units || !units.length) return "N/A";
    const dates = units.map(u => u.createdAt ? new Date(u.createdAt).getTime() : 0).filter(d => d > 0);
    ...
    <div className="space-y-6 text-[#E6EDF3] animate-fade-in pb-10">
      ...
      <section className="flex justify-between items-center bg-[#161B22]/40 p-4 border border-white/5 rounded-xl">
        <div>
          ...
          <p className="text-xs font-mono text-[#8B949E] mt-0.5">Unit Configuration Interface • [v.2.4.0]</p>
        </div>
        <button 
          onClick={downloadCSV}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label className="text-[10px] font-mono text-[#E6EDF3] uppercase tracking-wider mb-2 block">
                  Unit Measurement Label <span className="text-red-400">*</span>
                </label>
                <input 
                  type="text" 
                  name="unitName"
              ...
              <div className="bg-[#1C2128] border border-white/5 p-3 rounded-lg flex items-start gap-2.5 text-[10px] text-[#8B949E]">
                ...
                <span>Ensure unit labels are standardized across all entries to prevent data fragmentation in reporting modules.</span>
              </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-white/5">
                  {paginatedUnits.map((unit) => (
                    <tr key={unit._id} className="hover:bg-[#2563EB]/5 transition-colors group">
                      <td className="px-6 py-3 text-sm text-[#8B949E] font-mono whitespace-nowrap">{unit.recordId}</td>
                      <td className="px-6 py-3 text-sm font-bold text-[#E6EDF3] whitespace-nowrap">{unit.unitName}</td>
                      <td className="px-6 py-3 text-xs text-[#8B949E] font-mono whitespace-nowrap">
                        {unit.updatedAt ? new Date(unit.updatedAt).toLocaleDateString() : 'N/A'}
                      </td>
                      {(canEdit || canDelete) && (
              ...
              <div className="p-12 text-center text-[#8B949E] font-mono text-xs">
                {searchTerm ? 'No matching measurement logs found.' : 'Telemetry ledger empty. Configure new measurement metrics.'}
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-[#161B22] border border-white/5 rounded-xl p-4 flex items-center gap-4 skeuo-shadow">
          ...
          <div>
            <p className="text-[9px] font-mono text-[#8B949E] uppercase tracking-wider">Total Units</p>
            <h4 className="text-xl font-bold tracking-tight text-[#E6EDF3] mt-0.5">
              {loading ? '...' : String(units.length).padStart(2, '0')}
            </h4>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#161B22] border border-white/5 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-[#E6EDF3] mb-2 font-mono uppercase tracking-wider">Delete Unit Entry</h3>
              <p className="text-sm text-[#8B949E] mb-6">
                Are you sure you want to delete <span className="font-bold text-[#E6EDF3] font-mono">{unitToDelete?.recordId} ({unitToDelete?.unitName})</span>? This action cannot be undone and will fail if the unit is linked to existing items.
              </p>
