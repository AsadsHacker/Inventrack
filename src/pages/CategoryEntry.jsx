...
const CategoryEntry = () => {
  ...
  const resetForm = () => { 
    setIsEditing(false); 
    setEditId(null); 
    setFormData({ recordId: 'Auto-generated', categoryName: '' }); 
  };
  ...
  const downloadCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'inventory_categories.csv');
  ...
  const getLastIngestion = () => {
    ...
    const dates = data.map(c => c.createdAt ? new Date(c.createdAt).getTime() : 0).filter(d => d > 0);
    ...
    <div className="space-y-6 text-[#E6EDF3] animate-fade-in pb-10">
      ...
      <section className="flex justify-between items-center bg-[#161B22]/40 p-4 border border-white/5 rounded-xl">
        <div>
          ...
          <p className="text-xs font-mono text-[#8B949E] mt-0.5">Category Configuration Interface • [v.2.4.0]</p>
        </div>
        <button 
          onClick={downloadCSV}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                placeholder="E.g. RAW MATERIAL, SPARE PARTS"
                required
            ...
            <div className="bg-[#1C2128] border border-white/5 p-3 rounded-lg flex items-start gap-2.5 text-[10px] text-[#8B949E]">
              ...
              <span>Ensure category labels are standardized to support structured taxonomies inside global reporting parameters.</span>
            </div>
          ...
          <div className="overflow-x-auto">
            {loading ? (
                ...
                <tbody className="divide-y divide-white/5">
                  {paginatedCategories.map((item) => (
                    <tr key={item._id} className="hover:bg-[#2563EB]/5 transition-colors group">
                      ...
                      <td className="px-6 py-3 text-sm font-bold text-[#E6EDF3] whitespace-nowrap">{item.categoryName}</td>
              ...
              <div className="p-12 text-center text-[#8B949E] font-mono text-xs">
                {searchTerm ? 'No matching category records logs found.' : 'Telemetry ledger empty. Configure new taxonomy categories.'}
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        ...
        <div className="bg-[#161B22] border border-white/5 rounded-xl p-4 flex items-center gap-4 skeuo-shadow">
          ...
          <div>
            <p className="text-[9px] font-mono text-[#8B949E] uppercase tracking-wider">Total Categories</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-[#161B22] border border-white/5 rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-lg font-bold text-[#E6EDF3] mb-2 font-mono uppercase tracking-wider">Delete Category Entry</h3>
              <p className="text-sm text-[#8B949E] mb-6">
                Are you sure you want to delete <span className="font-bold text-[#E6EDF3] font-mono">{itemToDelete?.recordId} ({itemToDelete?.categoryName})</span>? This action cannot be undone and will fail if any items are assigned to this category.
              </p>
